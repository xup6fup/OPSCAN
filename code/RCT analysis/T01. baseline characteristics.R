...
intrested_var <- c('total_CXR_group', 'positive_prop_group',
                   'GENDER', 'AGE', 'AGE_group', 'BMI', 'BMI_group',
                   'Fracture', 'Fracture-Hip', 'Fracture-Wrist', 'Fracture-Vertebra', 'Fracture-Shoulder', 'Fracture-LowerLeg', 'Fracture-Osteoporosis', 
                   'SecondaryOsteoporosis', 'Hyperparathyroidism', 'Hyperthyroidism', 'MultipleMyeloma', 'CushingDisease', 'CeliacDisease',
                   'RheumatoidArthritis', 'Steroid')

data_path <-  './data/RCT analysis/RCT data.RData'
table_path <- './result/Table 01.doc'
...
sub_group <- follow_data

table_list[[1]] <- Table1(X = follow_data[,'Group'], Y.matrix = follow_data[,intrested_var], x.name = "Randomization")
