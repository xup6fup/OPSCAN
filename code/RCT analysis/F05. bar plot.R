...
data_list[[1]] <- follow_data[follow_data[,'Group'] %in% 'Low risk',]
data_list[[2]] <- follow_data[follow_data[,'Group'] %in% 'Control',]

names(data_list) <- c('AI-CXR low-risk group', 'AI-CXR high-risk group (Control)')
...
for (m in 1:length(data_list)) {
  ...
  if (m %in% 1) {risk <- 'low'} else {risk <- 'high'}
  
  group_dat.1 <- data.frame(x = c(1, 1, 1),
                            x_name = paste0('AI-CXR ', risk, ' risk (n = ', nrow(sub_data), ')'),
  ...
  sub_sub_data <- sub_data[sub_data[,'DXA_type'] %in% 1L,]
  
  group_dat.2 <- data.frame(x = c(2, 2, 2),
                            x_name = paste0('DXA by physicians (n = ', nrow(sub_sub_data), ')'),
  ...
  sub_sub_data <- sub_data[sub_data[,'DXA_type'] %in% 1L & sub_data[,'OP_type'] %in% 2L,]
  ...
  for (i in 1:2) {
    ...
    current_pos <- which(group_dat[,'group'] %in% c('DXA by physicians', 'Osteoporosis') & group_dat[,'x'] %in% i)
  ...
  gg_p <- gg_p + guides(fill = guide_legend(nrow = 8, byrow = TRUE, title = ""))
  ...
  if (m %in% 2) {
    ...
    gg_p <- gg_p + theme(legend.position = "right")
...
final_p <- ggdraw()
final_p <- final_p + draw_plot(bar_p_list[[1]], x = 0, y = 0, width = 0.4, height = 1)
final_p <- final_p + draw_plot(bar_p_list[[2]], x = 0.4, y = 0, width = 0.6, height = 1)

png(plot_path, width = 720, height = 640)
